...
namespace spin {
  ...
  void pointer_draw() {
    ...
    polygonColor(bgwin->win,x,y,4,0x606060ff);
...
void disp_nr(SDL_Surface *win,int nr,int y_off) {
  draw_ttf->draw_string(win,"num...",Point(4,y_off));
  char buf[20];
  sprintf(buf,"(%s) (id=%d)",the_num,nr);
  draw_ttf->draw_string(win,buf,Point(40,y_off));
}
...
void disp_topw() {
  ...
  SDL_FillRect(top_win->win,&bg,cWhite);
  red_font->draw_string(top_win->win,txt,Point(250,68)); // 2 pixels upwards
...
int main(int ac,char** av) {
  ...
  sl1=new VSlider(0,1,Rect(5,40,0,35),10,15,"vslider2", // parent will be set later
  ...
  top_win=new TopWin("Test SDL-widgets",Rect(100,100,380,500),0,SDL_RESIZABLE,disp_topw,set_icon);
  ...
  new Button(top_win,0,Rect(5,50,90,0),"set slider, rbut",
    [](Button *but) {
  ...
  rbw=new RButtons(bgwin,Style(1,0),Rect(5,90,60,3*TDIST),"rbuttons",false,[](RButtons* rb,int nr,int fire) {
    printf("rbw: maybe_z=%d nr=%d fire=%d\n",rb->maybe_z,nr,fire);
  });
  rbw->add_rbut("1."); rbw->add_rbut("2."); rbw->add_rbut("three");
  rbw->add_rbut(Label([](SDL_Surface *win,int nr,int y_off) {
      draw_ttf->draw_string(win,i2s(nr+1),Point(16,y_off));
      boxColor(win,26,y_off+5,36,y_off+9,0xff0000ff);
    })
  );
  rbw->value()=0;
  v_scr=new VScrollbar(bgwin,Style(1,0,5),Rect(70,90,0,3*TDIST),4*TDIST,
    [](VScrollbar *sb) {
  ...
  sl2=new VSlider(top_win,0,Rect(50,190,0,70),10,17,"vslider",
    [](VSlider* sl,int fire,bool rel) {    
          set_text(sl->text,"%d",sl->value());
      if (!bgwin) return;
  ...
  sl3=new HVSlider(top_win,0,Rect(120,190,60,60),Int2(0,0),Int2(4,4),"hvslider",
  ...
  static Lazy *lazy_scr,
              *lazy_sl;
/* without lazy_sl:
  sl4=new HSlider(top_win,0,Rect(210,190,60,0),0,5,"hslider1",
...
*/
  sl4=new HSlider(top_win,0,Rect(210,190,60,0),0,5,"hslider1",
    [](HSlider* sl,int fire,bool rel) {
      //printf("sl4 cmd: done=%d pause=%d rel=%d\n",lazy_sl->cmd_done,lazy_sl->pause,rel);
      lazy_sl->cmd_done=false;
      if (!lazy_sl->pause || rel) {
        lazy_sl->pause=true;
        lazy_sl->cmd_done=true;
        printf("%s: val=%d rel=%d\n",sl->title_str,sl->value(),rel);
        set_text(sl->text,"%d",sl->value());
        lazy_sl->kick();
      }
    },true);
  sl4->lab_left="0"; sl4->lab_right="5";
  lazy_sl=new Lazy([](int) {
    sl4->cmd(sl4,1,false); sl4->draw_blit_upd();
  },1000);
  ...
  txt_mes=new Message(top_win,2,"scrollbar:",Point(50,h_scr->area.y-16));
  ...
  rgb1=new HSlider(rgb_win,1,Rect(5,10,top_win->tw_area.w-10,0),0,64,0,
    [](HSlider* sl,int fire,bool rel) {
      col_R=val2col(sl); set_text(sl->text,"red = %x",col_R);
      rgb_cmd(rel);
    },true);
  rgb2=new HSlider(rgb_win,1,Rect(5,42,top_win->tw_area.w-10,0),0,64,0,
    [](HSlider* sl,int fire,bool rel) {
      col_G=val2col(sl); set_text(sl->text,"green = %x",col_G);
      rgb_cmd(rel);
    },true);
  rgb3=new HSlider(rgb_win,1,Rect(5,74,top_win->tw_area.w-10,0),0,64,0,
    [](HSlider* sl,int fire,bool rel) {
