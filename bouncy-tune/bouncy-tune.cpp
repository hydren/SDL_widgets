...
void Instrument::draw() {
  ...
  for (i=0; i <= Nend; i++) {
    ...
    if (n->fixed) {
      ...
      filledCircleColor(bview->win,n->d_x+n->nom_x,n->d_y+n->nom_y,rr,0xff6060ff);
      if (i==active_mass)
...
void ScSection::draw_sect(int lnr,int snr) {
  ...
  boxColor(scoreview->win,x,y-2,x+sect_len-2,y+2,0xffffffff); // erase section
  if (lcol)
    hlineColor(scoreview->win,x,x+sect_len,y,lcol==2 ? 0xff : 0xb0b0b0ff); // needed if stacc or not occupied
  if (occ())
    draw_playsect(x,y);
...
void ScSection::draw_ghost(int lnr,int snr,bool erase) {
  ...
  int y=scv_yoff+lnr*sclin_dist,
      lcol=line_col[lnr%12];
  boxColor(scoreview->win,x,y-2,x+sect_len-2,y+2,erase ? 0xffffffff : 0xa0a0a0ff);
  if (lcol && erase)
    hlineColor(scoreview->win,x,x+sect_len,y,lcol==2 ? 0xff : 0xb0b0b0ff);
  scoreview->blit_upd(rp(x,y-2,sect_len,5));
}
...
int main(int argc,char** argv) {
  ...
  bview=new BgrWin(top_win,Rect(190,20,bg_w,bg_h),"physical model",0,bouncy_down,bouncy_moved,bouncy_up,calc_color(0xe4e4ff));
