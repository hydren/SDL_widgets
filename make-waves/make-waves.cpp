...
void draw_wform(BgrWin *bgw,WavF *wf) { // supposed: bgw->parent = top_window
  ...
  for (i=0;i<wf->ptbuf.end;++i) {
    int x=wf->ptbuf[i].x,
        val=wf->ptbuf[i].y;
    circleColor(bgw->win,x,we::pt2y(val),2,0xff);
    if (i==0) circleColor(bgw->win,bg_w-1,we::pt2y(val),2,0xff);
  }
  int val2=we::pt2y(wf->wform[127]);
  for (i=0;i<bg_w;++i) {
    int val1=val2;
    val2=we::pt2y(wf->wform[i]);
    lineColor(bgw->win,i,val1,i+1,val2,0xff);
  }
  bgw->blit_upd();
}
...
//    ind=int(aa*ind + (1.-aa)/dim*ind*ind); if (ind<0) ind+=dim; else if (ind>=dim) ind-=dim;
    return buf[ind];
  }
} sinus;
/*
...
void bouncy_disp(int) {
  ...
  for (i = 0; i < com::NN-1; i++) {
    ...
    lineColor(bgw3->win, s->a->d_x, s->a->d_y, s->b->d_x, s->b->d_y,0xff);
  }
  for (i = 0; i < com::NN; i++) {
    Node *n = bouncy->nodes+i;
    if (n->fixed)
      boxColor(bgw3->win,n->d_x-5,n->d_y-10,n->d_x+5,n->d_y+10,0x00ff00ff);
    else {
      int rrx=int(sqrt(8 * n->mass/com::asym)),
          rry=int(sqrt(8 * n->mass*com::asym));
      filledEllipseColor(bgw3->win,n->d_x,n->d_y,rrx,rry,0xff6060ff);
    }
    if (i==com::pickup)
      filledCircleColor(bgw3->win,n->d_x,n->d_y,2,0xff);
  }
  bgw3->blit_upd();
};
...
Waves::Waves():
  ...
  am_amount=new HSlider(waves_ctr,1,Rect(10,130,60,0),0,5,"AM amount",
  ...
  fm_amount=new HSlider(waves_ctr,1,Rect(10,130,80,0),0,9,"FM amount",
    [](HSlider* sl,int fire,bool rel) {
      float fmval[10]={ 0.,0.1,0.2,0.3,0.5,0.7,1.,1.5,2.,3. };
      set_text(sl->text,"%.1f",fmval[sl->value()]);
      if (waves)
  ...
  ab_detune=new HSlider(waves_ctr,1,Rect(100,130,60,0),0,5,"A/B detune",
    [](HSlider* sl,int fire,bool rel) {
...
Bouncy::Bouncy():
  ...
  asym_xy=new HSlider(bouncy_ctr,1,Rect(8,58,100,0),0,8,"x/y asymmetry",
...
Harmonics::Harmonics():
  ...
  harm_freq=new HSlider(harm_ctr,1,Rect(10,20,100,0),0,11,"frequency",
    [](HSlider* sl,int fire,bool rel) {
  ...
  harm_offset=new HSlider(harm_ctr,1,Rect(10,58,100,0),-6,6,"sel: freq offset",
    [](HSlider* sl,int fire,bool rel) {
  ...
  ampl_mult=new HSlider(harm_ctr,1,Rect(10,96,100,0),0,8,"sel: ampl mult",
    [](HSlider* sl,int fire,bool rel) {
  ...
  harm_dist=new HSlider(harm_ctr,1,Rect(10,132,60,0),0,4,"distorsion",
    [](HSlider* sl,int fire,bool rel) {
  ...
  harm_n_mode=new HSlider(harm_ctr,1,Rect(80,132,74,0),0,6,"noise mode",
    [](HSlider* sl,int fire,bool rel) {
      lfnoise::set_mode(sl->value());
      set_text(sl->text,"%s",lfnoise::n_mname);
    }
  );
  harm_clip=new HSlider(harm_ctr,1,Rect(164,132,60,0),0,4,"soft clipping",
    [](HSlider* sl,int fire,bool rel) {
...
void FilterTest::draw_fresponse() {
  ...
  for (freq=log_f ? range/10 : 0;
    ...
    if (tfer)
      vlineColor(bgw5->win,xpos,max(0,bgw5->tw_area.h-tfer-1),bgw5->tw_area.h-1,freq>range ? 0x808080ff : 0xff);
    if (!x_nyq && freq>22050.) x_nyq=xpos;
  }
  if (x_nyq && x_nyq<wid)
    vlineColor(bgw5->win,x_nyq,0,10,0xff0000ff);
  bgw5->blit_upd();
  updating=false;
}
...
FilterTest::FilterTest():
  ...
  filt_mode=new HSlider(filt_ctr,1,Rect(10,130,74,0),0,6,"mode",
    [](HSlider* sl,int fire,bool rel) {
      int fmod=sl->value();
      if (filt) {
  ...
  filt_range=new HSlider(filt_ctr,1,Rect(145,18,80,0),0,4,"range",
    [](HSlider* sl,int fire,bool rel) {
  ...
  filt_qfactor=new HSlider(filt_ctr,1,Rect(145,58,70,0),0,5,"filter Q",
    [](HSlider* sl,int fire,bool rel) {
  ...
  filt_freq=new HSlider(filt_ctr,1,Rect(92,130,138,0),0,12,"corner freq",
    [](HSlider* sl,int fire,bool rel) {
...
void Harmonics::draw_line(int ind) {
  ...
  if (lin->info)
    circleColor(bgw4->win,ind*ldist,3,2,0xf00000ff);
...
int main(int argc,char** argv) {
  ...
  top_win=new TopWin("Make Waves",Rect(100,100,300,390),SDL_INIT_AUDIO,0,topw_disp);
